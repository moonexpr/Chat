...
var WebSocket = /** @class */ (function (_super) {
    ...
    WebSocket.prototype.bindWebSocket = function (ws) {
        ...
        ws.on('connect', function (client) {
            cli_1.default.connect("New connection from " + client.remoteAddress);
            _this.emit('connect', client);
        });
        ws.on('close', function (client, reason, desc) {
            cli_1.default.disconnect("Received farewell from " + client.remoteAddress);
            _this.emit('close', client, reason, desc);
        });
